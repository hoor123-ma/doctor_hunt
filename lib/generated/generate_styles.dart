...
void main() {
  final List<String> colors = ['primaryColor', 'greyColor',"whiteColor"];
...
Future<void> generateWeightAtoms(IOSink sink) async {
  sink.writeln('''
...
void generateColors(IOSink sink, List<String> colors, String nameColorsClass) {
  ...
  for (final String color in colors) {
    sink.writeln(
      '  TextStyle get $color => '
      'copyWith(color: $nameColorsClass.$color);',
    );
  }
...
void generateFontSizes(IOSink sink, List<int> fontSizes) {
  ...
  for (final int size in fontSizes) {
    sink.writeln(
      '  TextStyle get px$size => '
      'copyWith(fontSize: $size.0);',
    );
  }
